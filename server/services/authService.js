...
const sendOtp = async (phone) => {
    // Try exact match first, then try local format (0...)
    let [users] = await UserModel.findByPhone(phone);
    if (!users.length) {
        const local = '0' + phone.replace(/^\+\d{1,3}/, '');
        [users] = await UserModel.findByPhone(local);
    }
